    ...
    public function getTabs(): array
    {
        return [
            ...
            'open' => Tab::make(__('ui.open'))
                ...
                ->modifyQueryUsing(fn ($q) => $q->where('status', TaskStatusEnum::OPEN)),
            ...
            'assigned' => Tab::make(__('ui.assigned'))
                ...
                ->modifyQueryUsing(fn ($q) => $q->where('status', TaskStatusEnum::ASSIGNED)),
            ...
            'in_progress' => Tab::make(__('ui.in_progress'))
                ...
                ->modifyQueryUsing(fn ($q) => $q->where('status', TaskStatusEnum::IN_PROGRESS)),
            ...
            'resolved' => Tab::make(__('ui.resolved'))
                ...
                ->modifyQueryUsing(fn ($q) => $q->where('status', TaskStatusEnum::RESOLVED)),
            ...
            'closed' => Tab::make(__('ui.closed'))
                ...
                ->modifyQueryUsing(fn ($q) => $q->where('status', TaskStatusEnum::CLOSED)),

            'breached' => Tab::make(__('ui.sla_breached'))
                ->badge(fn () => Ticket::query()->where('sla_breached', true)->count())
                ->badgeIcon('heroicon-o-exclamation-triangle')
                ->badgeColor('danger')
                ->modifyQueryUsing(fn ($q) => $q->where('sla_breached', true)),
        ];
    }
}
