...
### What it does
- **Sweep loop** — finds active-status tickets with `sla_deadline < now()` and `sla_breached = false`, flips the column, dispatches `TicketSlaBreached` event, fans out `SlaBreachedNotification` to assignee + group supervisor
- **Warning loop** — finds active-status tickets with `sla_deadline > now()` and `sla_breached = false` whose elapsed-time fraction has crossed 80%, fans out `SlaWarningNotification`
...
### Recipients
- `slaRecipients()` returns `[$ticket->employee->user, $ticket->group->employee->user]` (assignee + group supervisor) — no admin fallback by design; admins see breach state through the panel widgets and the navigation badge
- Both walks go through `Employee->user` (LDAP-bound by email); missing relations short-circuit silently
